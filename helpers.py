...
def make_table(doc, table_name, column_names, data=()):
    ...
    if col_lengths <1:
        ...
        message = ('You are trying to create an empty table without ' +
                   'any data, this is useless, process stopped!')
...
def _delete_rows_(table, rowfilter):
    ...
    heading = 'Deleting from table {}'.format(table.Name)

    message = "Remove?" #.format(len(rowfilter))
